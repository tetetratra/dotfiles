...
- Q1: 与えられた依頼やチケット中に、タスクの背景や目的が書いてありますか？以下のチェックリストで確認してください
  - チェックリスト
    - [ ] タスクのビジネス的な意義・必要性が書いてある
    - [ ] タスクが起票された経緯が書いてある
    - [ ] タスクで達成したいことと、その理由が書いてある
  - すべてチェックできる: 次へ
  - 1つでもチェックできない: 「背景・目的の整理」「残りロードマップの再検討」を追加し、終了

- Q2: 今回のタスクを進めるために、あなた（AI）は以下の情報を持っていますか？以下のチェックリストで確認してください
  - チェックリスト
    ...
    - [ ] 類似の実装例や前例を知っている
  - すべてチェックできる: 次へ
  - 1つでもチェックできない: 「既存システム・関連技術の調査」「残りロードマップの再検討」を追加し、終了
...
- Q8: [Q] 必要であればユーザーに、対応方針を進めるにあたって、どのようなステップに分けて進めるべきか意見を伺ってください
          その際、「ステップ分けの判断基準」セクションを参考に、あなた(AI)の意見も添えてください。
...
- **問いの明確化**: 漠然とした問いやイシューを、仮説・検証のサイクルを通じて具体的な結論を導く形に整える
  - 推奨スキル: `dev-step--refine-question`
...
- **既存システム・関連技術の調査**: 今回のタスクに関連する「システムの仕組み」「技術・概念」「前例」などについて調査をし、意思決定の判断材料を揃える
  - 推奨スキル: `dev-step--research-case-study`(他社事例調査が必要な場合)
...
- **レビュー修正**: レビューを受けてその対応を行う
  - 推奨スキル: `dev-step--respond-ci-reviews`(CIレビューbotへの対応の場合)、 `dev-step--followup-pr`(PR全体のCI失敗・指摘・コンフリクトを横断対応する場合)
...
タスクのステップが上流から下流に一直線に進まない場合、子タスクに分解したほうが良いです。
例えば「Aの調査→Aの設計→Aの実装→Bの調査→Bの設計→Bの実装」というステップがあった場合、
