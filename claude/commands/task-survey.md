...
以下の順序でセクションを構成（6ステップに沿った構成。FAQは含めません）:
...
#### 4.3 完了の判断基準
以下の質問に答えられれば、調査完了（6ステップすべてをカバー）:
- [ ] この機能を一言で説明できる（概要・目的）
...
- [ ] 6つのセクション（概要・目的、基本的な使い方、詳細な機能説明、具体的なユースケース、技術的な仕組み、応用・カスタマイズ）すべてが揃っているか
...
- [ ] 閉じカッコ（end, } など）を省略しているか
- [ ] 非同期処理が `[非同期処理]` で明記されているか
