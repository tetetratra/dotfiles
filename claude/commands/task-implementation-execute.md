...
注意：このコマンドは、前段までに立てた実装計画に従って実装するものです。
計画の変更や追加の検討は行わず、決定された計画に従って実装することが目的です。
...
- 実装計画のステップに従って順番に実装する
- 各ステップ完了時にテストとlintチェックを実行し、結果を確認する
- 各ステップ完了時にコミットする
- マイルストーン到達時は必ず実装を停止し、ユーザーにレビューを依頼する
- 計画通りに進まない場合は理由を明示し、計画の見直しを提案する
