...
それでも不明点が残る場合は、ユーザーに質問を投げて、応答を終了してください。
...
まず、作業ステップを構築する際の参考として、一般的な作業ステップの種類を以下に示します。
...
- タスクの要件はすでに定義されている？もしくは自明(= タスクの目的そのまま)か？
  - yes: 次へ
  - no: 「要件定義」を追加し、次へ

- タスクの対応方針は自明か？もしくはすでに決定されている？
  - yes: 次へ
  - no: 「対応方針決め」「残りステップの再検討」を追加し、終了

- [T] 対応方針について、改めて、その対応方針を進めるにあたって複数の子タスクに分解して実行すべきと思うか？
  - yes(= 子タスクに分解すべき): 「タスク分解」というステップを追加し、終了
  - no(= このタスク1個で十分): 次へ
